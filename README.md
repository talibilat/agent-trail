...
The browser event inspector presents each context path, line range, symbol, and reading actor linked by `informed_by` directly on the selected change hunk; an end-only range is displayed as `path:?-end`, and unrelated context links are not attributed as informing evidence.
