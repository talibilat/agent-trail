...
A verification category is present only when a `verified_by` relationship resolves to a finished verification with a non-empty command, either directly or through a resolved start event; other links to verifications remain visible, while an outcome or bare start event alone does not satisfy it.
