...
Resolved canonical tool links include `chronology` relative to the earliest attributable decision, identifying that boundary as `decision_event_id`, or to the applied change when no decision is attributable; equal timestamps across independent emitters are reported as `undetermined`.
