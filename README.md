...
Compaction links from a change also include `chronology` relative to the earliest attributable decision and identify that boundary as `decision_event_id`, or use the applied change when no decision is attributable; equal timestamps across independent emitters are reported as `undetermined` rather than claimed to establish before or after ordering.
