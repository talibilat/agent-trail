...
A `preceded_by` tool event timestamped after the earliest attributable `change.proposed` decision or after its change remains inspectable but produces a temporal diagnostic and keeps coverage incomplete; equal timestamps remain valid because they do not establish contradictory ordering.
