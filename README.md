...
The browser event inspector presents the actor for compactions linked by `informed_by`, or labels the compacting actor unknown when its ID is blank, plus repository locators with optional symbols, source actors, and missing or wrong-kind source diagnostics linked by `summarizes` directly on the selected change hunk; unrelated outer and source relationships are not attributed as compacted context.
