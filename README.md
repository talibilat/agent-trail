...
To identify repository or documentation evidence, emit a `context.read` event with a non-blank `attributes.context.path`, optional non-negative integer `line_start` and `line_end` fields, and an optional non-blank string `symbol`, then reference it from the change event with an `informed_by` relationship.
...
Resolved links to that event include `compaction.sources` with each distinct resolved source event, its kind, actor, and any context-read locator, plus `compaction.unresolved` for each distinct missing source event.
This preserves the observable compaction boundary without capturing private reasoning or summary contents.
The browser event inspector presents the actor for compactions linked by `informed_by`, plus source kinds or repository locators with optional symbols, source actors, and missing source IDs linked by `summarizes` directly on the selected change hunk; unrelated outer and source relationships are not attributed as compacted context.
