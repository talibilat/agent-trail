...
A verification category is present only when a `verified_by` relationship resolves to a finished verification with a non-empty command, either directly or through a resolved start event; other links to verifications remain visible, and a validated outcome or bare start event alone does not satisfy coverage.
...
Coverage is `complete` only when all five core categories are present, every direct, compacted, or verification-lifecycle reference resolves, and every verification is known to predate the change.
Otherwise `missing` identifies absent categories, `unknown_test_origin_count` identifies verifications without valid provenance, and `same_agent_test_count` identifies tests written by the implementing agent, without assigning a subjective confidence score.
...
To identify a motivating requirement, emit a `requirement.observed` event with non-empty `attributes.requirement.id` and `attributes.requirement.text` strings, then reference it from the change event with a `motivated_by` relationship.
